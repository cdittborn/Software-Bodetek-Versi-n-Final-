...
import { autorizarCarpeta } from "@/lib/storage/autorizarCarpeta";
...
const presignSchema = z.object({
  ...
  keyObjetivo: z
    ...
    .regex(
      /^[a-z0-9/_.-]+$/i,
      "keyObjetivo solo puede incluir letras, números, /, _, . y -",
    )
    .optional(),
});
...
export async function POST(request: Request) {
  try {
    ...
    const { nombreArchivo, tipoArchivo, carpeta, keyObjetivo } = parsed.data;
    ...
    let key: string;
    if (keyObjetivo) {
      const keyNormalizada = keyObjetivo.replace(/^\/+/, "");
      const prefijo = `${carpetaNormalizada}/`;
      if (
      ...
      key = `${carpetaNormalizada}/${randomUUID()}${ext}`;
    }
