#!/usr/bin/env bash
# Aplica estado independiente en producción: BEGIN → migración → COMMIT.
# Después verifica en OTRA conexión (solo SELECT).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SQL="$ROOT/scripts/aplicar-estado-independiente-commit.sql"
VERIFY="$ROOT/scripts/verificar-estado-independiente.sql"
...
if host.startswith("db.") and host.endswith(".supabase.co") and not has_ipv4(host, p.port or 5432):
    ...
    print("rewritten_to_session_pooler:", pool_host, file=__import__("sys").stderr)
print(url)
PY
)"
