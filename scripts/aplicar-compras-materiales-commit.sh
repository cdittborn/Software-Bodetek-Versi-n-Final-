#!/usr/bin/env bash
# Aplica compras de materiales en producción: BEGIN → CREATE → COMMIT.
# NO correr hasta que un humano confirme el dry-run.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SQL="$ROOT/scripts/aplicar-compras-materiales-commit.sql"
VERIFY="$ROOT/scripts/verificar-compras-materiales.sql"
